...
if __name__ == "__main__":
    ...
    # plotting
    f, axes = plt.subplots(1, 3, figsize=(11, 6))
    cell_list = ["AEC", "LYM", "OC"]
    sns.violinplot(y="Area", x="Label", hue="Source", split=True, inner=None, order=cell_list, data=cell_fea_df, orient='v', ax=axes[0])
    sns.violinplot(y="Intensity", x="Label", hue="Source", split=True, inner=None, order=cell_list, data=cell_fea_df, orient='v', ax=axes[1])
    sns.violinplot(y="Roundness", x="Label", hue="Source", split=True, inner=None, order=cell_list, data=cell_fea_df, orient='v', ax=axes[2])
    ...
    plt.savefig(all_cell_plot_path, transparent=False, dpi=300)